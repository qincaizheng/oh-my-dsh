# oh-my-dsh — DSH Integration Pack

[中文](./README.zh.md) | English

One-command integration pack for DeepSeek Harness (DSH): 18 curated plugin groups plus out-of-the-box configuration — the Web UI suite, a coding toolchain, the approve-for-me auto-approval tier (codex-guardian-style review + "Approve For Me" sandbox option), and a compatibility fix layer for third-party plugins.

## What's inside

See [PLUGINS.md](./PLUGINS.md) for the full, authoritative plugin list (package name / source / install method / mount per entry).

## Quick start

The authoritative installation manual is [INSTALL.md](./INSTALL.md). Hand the prompt below to any AI / Agent and it will read the manual and perform the full install:

```text
Read https://github.com/qincaizheng/oh-my-dsh/blob/main/INSTALL.md and treat it as the single authoritative installation manual. Install and configure the full integration pack on this machine's DeepSeek Harness environment — into the dedicated `oh-my-dsh` profile (created by copying the default `web` profile per the manual, never touching the default profile itself): compare PLUGINS.md against this machine's installed plugins first, install only the missing plugins (skip everything already installed), apply the config additions, apply the disabled-list handling from the manual (DISABLED.md) and tell me what was disabled and how to re-enable it, run the repairs and verifications from the manual, restart the pack's profile (`dsh --profile oh-my-dsh`), and report the result back to me.
```

## Repository layout

```text
.
├── README.md      # English intro (default) + install prompt for AI agents
├── README.zh.md   # Chinese version of README.md
├── PLUGINS.md     # Authoritative plugin list: package name / source / install method / mount
├── DISABLED.md    # Default-disabled plugin list (auto-applied on install/update, with re-enable instructions)
└── INSTALL.md     # Installation manual: compare → install missing → config → verify → rollback
```

## Notes

- Verified against DSH 0.1.0-rc.6. Some older plugins (agent-teams / toolkit originate from the rc.1 era) were rebuilt/relinked against rc.6 — **after upgrading DSH, re-run the repairs and rebuilds in INSTALL.md §4**.
- Third-party compatibility intelligence: <https://github.com/AdamPlatin123/awesome-dsh-plugins> (daily compatibility matrix), <https://github.com/0xsline/awesome-deepseek-harness> (hand-curated directory).
- `dsh-upstream-fixes` is this pack's fix layer, originally repairing two crash points from the old auto-approval / sidechain plugins; with auto-approval replaced by approve-for-me and sidechain default-disabled it is now inert, but keep it installed for compatibility — see INSTALL.md §6.